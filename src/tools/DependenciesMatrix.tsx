    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.entries(DEP_COLORS).map(([key, cls]) => (
          <div key={key} className={`px-2.5 py-1 rounded-md text-sm font-medium font-sans border ${cls}`}>
            {key.replace("-", " ")}
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-6">
        ...
        <div className="glass-card p-4 md:p-5 space-y-3">
          ...
          {items.map((item, i) => (
            <div key={item.id} className="flex items-center gap-2 border border-border/40 rounded-lg p-2.5">
              <span className="text-sm text-foreground/60 font-mono shrink-0">{String(i + 1)}</span>
              <input type="text" value={item.name} onChange={(e) => setItems((prev) => prev.map((x) => x.id === item.id ? { ...x, name: e.target.value } : x))}
                className="flex-1 bg-transparent border-b border-border/60 py-0.5 text-sm text-foreground font-sans placeholder:text-foreground/60 focus:outline-none focus:border-gold/50 transition-colors" placeholder="Item name" />
              <input type="text" value={item.phase} onChange={(e) => setItems((prev) => prev.map((x) => x.id === item.id ? { ...x, phase: e.target.value } : x))}
                className="w-24 bg-transparent border-b border-border/60 py-0.5 text-sm text-foreground/75 font-sans placeholder:text-foreground/50 focus:outline-none focus:border-gold/50 transition-colors text-right" placeholder="Phase" />
              <button onClick={() => removeItem(item.id)} className="text-foreground/40 hover:text-red-400 transition-colors" aria-label="Remove"><Trash2 size={11} /></button>
            </div>
          ))}
        </div>
        ...
        <div className="glass-card p-4 md:p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="font-serif font-semibold text-foreground text-base">Links</h2>
            <button onClick={addDep} className="flex items-center gap-1 text-sm font-medium text-gold-dark hover:text-gold transition-colors font-sans">
              <Plus size={14} /> Add Link
            </button>
          </div>

          {deps.length === 0 && <p className="text-sm text-foreground/60 font-sans text-center py-4">No dependencies yet</p>}

          {deps.length > 0 && deps.map((d, i) => {
            const from = items.find((x) => x.id === d.fromId);
            const to = items.find((x) => x.id === d.toId);
            return (
              <div key={d.id} className="flex items-center gap-1.5 border border-border/40 rounded-lg p-2 text-sm font-sans">
                <span className="text-sm text-foreground/50 font-mono shrink-0">{i + 1}</span>
                <span className="flex-1 truncate text-foreground font-medium">{from?.name || "?"}</span>
                <ArrowRight size={11} className="text-foreground/60 shrink-0" />
                ...
                <select value={d.type} onChange={(e) => setDeps((prev) => prev.map((x) => x.id === d.id ? { ...x, type: e.target.value as DepType } : x))}
                  className="bg-transparent text-sm text-foreground/75 font-sans focus:outline-none">
                  <option value="blocks">blocks</option>
                  <option value="depends-on">depends-on</option>
                  <option value="related">related</option>
                </select>
                <button onClick={() => removeDep(d.id)} className="text-foreground/40 hover:text-red-400 transition-colors" aria-label="Remove"><Trash2 size={10} /></button>
              </div>
            );
          })}
        </div>
      ...
      {items.filter((i) => i.name.trim()).length > 0 && (
        <div className="glass-card-strong p-4 md:p-5">
          <h2 className="font-serif font-semibold text-foreground text-base mb-3">Dependency View</h2>
            ...
            {items.filter((i) => i.name.trim()).map((item) => {
              ...
              return (
                <div key={item.id} className="border border-border/30 rounded-lg p-3">
                  <p className="text-sm font-semibold text-foreground font-sans mb-1.5">{item.name}</p>
                  <div className="flex gap-4 text-sm font-sans">
                    {d.blocking.length > 0 && <div><span className="text-red-400 font-medium">Blocks:</span> <span className="text-foreground/75">{d.blocking.join(", ")}</span></div>}
                    {d.blockedBy.length > 0 && <div><span className="text-amber-400 font-medium">Blocked by:</span> <span className="text-foreground/75">{d.blockedBy.join(", ")}</span></div>}
                    {d.blocking.length === 0 && d.blockedBy.length === 0 && <span className="text-foreground/60">No dependencies</span>}
                  </div>
                </div>
              );
            })}
