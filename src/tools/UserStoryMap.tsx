  ...
  return (
    ...
    >
      ...
      <div className="glass-card p-4 md:p-5">
        <div className="flex items-center gap-4 text-sm font-sans text-foreground/75">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-sm text-foreground/70">← High priority</span>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-5 md:p-6 space-y-6 overflow-x-auto">
        <div className="flex items-center justify-between">
          <h2 className="font-serif font-semibold text-foreground text-base shrink-0">
            Story Map
          </h2>
          <button
            onClick={addActivity}
          ...
          {activities.map((activity) => (
            ...
            >
              ...
              <div className="bg-gold/10 border border-gold/30 rounded-lg p-2.5">
                <div className="flex items-center gap-1.5">
                  <GripVertical size={12} className="text-foreground/50 shrink-0" />
                  <input
                    type="text"
                    value={activity.name}
                    onChange={(e) => updateActivityName(activity.id, e.target.value)}
                    className="flex-1 bg-transparent text-sm font-semibold text-foreground font-sans border-b border-transparent focus:border-gold/50 focus:outline-none transition-colors"
                  />
                  <button
                    onClick={() => removeActivity(activity.id)}
                    className="text-foreground/50 hover:text-red-400 transition-colors shrink-0"
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-sm text-foreground/70 hover:text-gold-dark transition-colors font-sans mt-1.5"
                >
                  ...
                  .map((story, si) => (
                    ...
                    >
                      <div className="flex items-start gap-1">
                        <span className="text-[9px] text-foreground/60 font-mono mt-0.5 shrink-0">
                          {si + 1}
                        </span>
                        <div className="flex-1 min-w-0">
                          <input
                            ...
                            className="w-full bg-transparent text-sm text-foreground font-sans border-b border-transparent focus:border-gold/50 focus:outline-none transition-colors placeholder:text-foreground/60"
                          ...
                          <input
                            ...
                            className="w-full bg-transparent text-[9px] text-foreground/70 font-sans mt-0.5 border-b border-transparent focus:border-gold/30 focus:outline-none transition-colors placeholder:text-foreground/40"
                            placeholder="Notes (optional)"
                          />
                        </div>
                        <button
                          onClick={() => removeStory(activity.id, story.id)}
                          className="text-foreground/40 hover:text-red-400 transition-colors shrink-0 opacity-0 group-hover:opacity-100"
      ...
      >
        <h2 className="font-serif font-semibold text-foreground text-base mb-3">Map Summary</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="text-center p-3 rounded-lg bg-background/50">
            <p className="text-2xl font-serif font-bold text-foreground">{activities.length}</p>
            <p className="text-sm text-foreground/70 font-sans">Activities</p>
          </div>
          <div className="text-center p-3 rounded-lg bg-background/50">
            <p className="text-2xl font-serif font-bold text-foreground">{totalStories}</p>
            <p className="text-sm text-foreground/70 font-sans">User Stories</p>
          </div>
          <div className="text-center p-3 rounded-lg bg-background/50">
            <p className="text-2xl font-serif font-bold text-foreground">
              {activities.filter((a) => a.stories.length > 0).length}
            </p>
            <p className="text-sm text-foreground/70 font-sans">With Stories</p>
          </div>
          <div className="text-center p-3 rounded-lg bg-background/50">
            <p className="text-2xl font-serif font-bold text-foreground">
              {totalStories > 0
            ...
            <p className="text-sm text-foreground/70 font-sans">Avg Stories/Activity</p>
          </div>
        </div>
      </motion.div>
