    ...
    >
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground/50 font-sans">Quarter</p>
            </div>
          </div>
          <div className="text-center">
            <p className="text-3xl md:text-4xl font-serif font-bold gold-text">{progress.overall}%</p>
            <p className="text-[10px] text-muted-foreground/50 font-sans">Overall Progress</p>
          </div>
        </div>
      ...
      <div className="glass-card p-5 md:p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="font-serif font-semibold text-foreground text-sm">Objectives</h2>
          <button onClick={addObjective} className="flex items-center gap-1 text-xs font-medium text-gold-dark hover:text-gold transition-colors font-sans">
        ...
        {progress.objectives.map((obj, oi) => (
          <motion.div key={obj.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
            ...
            {/* Objective row */}
            <div className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground/50 font-mono shrink-0">O{String(oi + 1).padStart(2, "0")}</span>
              <input type="text" value={obj.name} onChange={(e) => updateObjective(obj.id, e.target.value)}
                className="flex-1 bg-transparent border-b border-border/60 py-0.5 text-sm font-semibold text-foreground font-sans placeholder:text-muted-foreground/40 focus:outline-none focus:border-gold/50 transition-colors" placeholder="Objective" />
              {obj.progress !== null && (
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="text-xs font-mono font-bold gold-text">{obj.progress}%</span>
                </div>
              )}
              <button onClick={() => removeObjective(obj.id)} className="text-muted-foreground/20 hover:text-red-400 transition-colors shrink-0" aria-label="Remove"><Trash2 size={12} /></button>
            </div>

            {/* Key Results */}
            <div className="ml-6 space-y-2">
              {obj.keyResults.length > 0 && (
                <div className="hidden sm:grid sm:grid-cols-[1fr_80px_80px_30px] gap-2 text-[10px] uppercase tracking-wider text-muted-foreground/40 font-sans pb-1 border-b border-border/20">
                ...
                return (
                  <div key={kr.id} className="grid sm:grid-cols-[1fr_80px_80px_30px] gap-2 items-center border border-border/30 rounded-lg p-2.5">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] text-muted-foreground/30 font-mono shrink-0">KR{ki + 1}</span>
                      <input type="text" value={kr.name} onChange={(e) => updateKR(obj.id, kr.id, "name", e.target.value)}
                        className="flex-1 bg-transparent text-xs text-foreground font-sans border-b border-transparent focus:border-gold/50 focus:outline-none transition-colors placeholder:text-muted-foreground/40" placeholder="Key result" />
                      <span className={`text-[10px] font-mono font-bold ${pct >= 80 ? "text-emerald-500" : pct >= 50 ? "text-amber-500" : "text-red-400"}`}>{pct}%</span>
                    </div>
                    <input type="number" value={kr.current} onChange={(e) => updateKR(obj.id, kr.id, "current", e.target.value)}
                      className="w-full text-center bg-transparent border-b border-border/60 py-0.5 text-xs text-foreground font-mono focus:outline-none focus:border-gold/50 transition-colors" min="0" step="0.1" />
                    <input type="number" value={kr.target} onChange={(e) => updateKR(obj.id, kr.id, "target", e.target.value)}
                      className="w-full text-center bg-transparent border-b border-border/60 py-0.5 text-xs text-foreground font-mono focus:outline-none focus:border-gold/50 transition-colors" min="0" step="0.1" />
                    <button onClick={() => removeKR(obj.id, kr.id)} className="text-muted-foreground/20 hover:text-red-400 transition-colors justify-self-center" aria-label="Remove"><Trash2 size={10} /></button>
                  </div>
                );
              })}

              <button onClick={() => addKR(obj.id)} className="flex items-center gap-1 text-[10px] text-muted-foreground/50 hover:text-gold-dark transition-colors font-sans ml-1">
